...
#import "ZYQStatus.h"
#import "FMDB.h"
@implementation ZYQStatusCacheTool
...
{
    for (ZYQStatus *status in statusArray) {
...
+ (void)addStatus:(ZYQStatus *)status
{
    ...
    [_queue inDatabase:^(FMDatabase *db) {
        // 1.获得需要存储的数据
        NSString *accessToken = [ZYQAccountTool account].access_token;
...
+ (NSArray *)statuesWithParam:(ZYQHomeStatusesParam *)param
{
    ...
    [_queue inDatabase:^(FMDatabase *db) {
        ...
        while (rs.next) {
            ...
            ZYQStatus *status = [NSKeyedUnarchiver unarchiveObjectWithData:data];
