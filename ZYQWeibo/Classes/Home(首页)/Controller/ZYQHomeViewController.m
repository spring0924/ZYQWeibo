...
@interface ZYQHomeViewController () <MJRefreshBaseViewDelegate>
@property (nonatomic, weak) ZYQTitleButton *titleButton;
@property (nonatomic, strong) NSMutableArray *statusFrames;
@property (nonatomic, weak) MJRefreshFooterView *footer;
@property (nonatomic, weak) MJRefreshHeaderView *header;
@end

@implementation ZYQHomeViewController
...
{
    // 1.封装请求参数
    ZYQUserInfoParam *param = [ZYQUserInfoParam param];
    param.uid = @([ZYQAccountTool account].uid);
    
    // 2.发送请求
    [ZYQUserTool userInfoWithParam:param success:^(ZYQUserInfoResult *result) {
        // 设置标题文字
        [self.titleButton setTitle:result.name forState:UIControlStateNormal];
        // 保存昵称
        ZYQAccount *account = [ZYQAccountTool account];
...
{
    // 1.封装请求参数
    ZYQHomeStatusesParam *param = [ZYQHomeStatusesParam param];
    if (self.statusFrames.count) {
        ZYQStatusFrame *statusFrame = [self.statusFrames lastObject];
        // 加载ID <= max_id的微博
        param.max_id = @([statusFrame.status.idstr longLongValue] - 1);
    }
    
    // 2.发送请求
    [ZYQStatusTool homeStatusesWithParam:param success:^(ZYQHomeStatusesResult *result) {
        // 创建frame模型对象
        NSMutableArray *statusFrameArray = [NSMutableArray array];
        for (ZYQStatus *status in result.statuses) {
            ZYQStatusFrame *statusFrame = [[ZYQStatusFrame alloc] init];
            // 传递微博模型数据
...
- (void)loadNewData
{
    ...
    ZYQHomeStatusesParam *param = [ZYQHomeStatusesParam param];
    if (self.statusFrames.count) {
        ZYQStatusFrame *statusFrame = self.statusFrames[0];
...
- (void)setupNavBar
{
    ...
    // 中间按钮
    ZYQTitleButton *titleButton = [ZYQTitleButton titleButton];
    // 图标
    [titleButton setImage:[UIImage imageWithName:@"navigationbar_arrow_down"] forState:UIControlStateNormal];
    // 位置和尺寸
    titleButton.frame = CGRectMake(0, 0, 0, 40);
    // 文字
    if ([ZYQAccountTool account].name) {
        [titleButton setTitle:[ZYQAccountTool account].name forState:UIControlStateNormal];
    } else {
    ...
    self.tableView.backgroundColor = ZYQColor(226, 226, 226);
    self.tableView.contentInset = UIEdgeInsetsMake(0, 0, ZYQStatusTableBorder, 0);
    self.tableView.separatorStyle = UITableViewCellSeparatorStyleNone;
}

- (void)titleClick:(ZYQTitleButton *)titleButton
{
...
{
    // 1.创建cell
    ZYQStatusCell *cell = [ZYQStatusCell cellWithTableView:tableView];
...
{
    ZYQStatusFrame *statusFrame = self.statusFrames[indexPath.row];
