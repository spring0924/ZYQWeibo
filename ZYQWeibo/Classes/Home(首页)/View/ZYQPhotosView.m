...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        // 初始化9个子控件
        for (int i = 0; i<9; i++) {
            ZYQPhotoView *photoView = [[ZYQPhotoView alloc] init];
...
- (void)photoTap:(UITapGestureRecognizer *)recognizer
{
    ...
    for (int i = 0; i<count; i++) {
        ...
        ZYQPhoto *ZYQphoto = self.photos[i];
        NSString *photoUrl = [ZYQphoto.thumbnail_pic stringByReplacingOccurrencesOfString:@"thumbnail" withString:@"bmiddle"];
...
- (void)setPhotos:(NSArray *)photos
{
    ...
    for (int i = 0; i<self.subviews.count; i++) {
        // 取出i位置对应的imageView
        ZYQPhotoView *photoView = self.subviews[i];
        ...
        if (i < photos.count) {
            ...
            CGFloat photoX = col * (ZYQPhotoW + ZYQPhotoMargin);
            CGFloat photoY = row * (ZYQPhotoH + ZYQPhotoMargin);
            photoView.frame = CGRectMake(photoX, photoY, ZYQPhotoW, ZYQPhotoH);
...
+ (CGSize)photosViewSizeWithPhotosCount:(int)count
{
    ...
    // 高度
    CGFloat photosH = rows * ZYQPhotoH + (rows - 1) * ZYQPhotoMargin;
    
    // 总列数
    int cols = (count >= maxColumns) ? maxColumns : count;
    // 宽度
    CGFloat photosW = cols * ZYQPhotoW + (cols - 1) * ZYQPhotoMargin;
